...
export class ValueLedger implements ValueLedgerBase {
  ...
  public static async deploy(
    ...
    recipe: ValueLedgerDeployRecipe
  ) {
   ...
   */
  public async approveValue(
    value: string,
    accountId: string
  ): Promise<Mutation> {
    accountId = this._provider.encoder.normalizeAddress(accountId as string);

    const approvedValue = await this.getApprovedValue(
      this.provider.accountId,
      accountId
    );
    if (
      !bigNumberify(value).isZero() &&
      !bigNumberify(approvedValue).isZero()
    ) {
  ...
  public async transferValue(
    recipe: ValueLedgerTransferRecipe
  ): Promise<Mutation> {
