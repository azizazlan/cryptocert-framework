import { Encode, Encoder } from "@cryptocert/ethereum-utils";
import {
  ProviderBase,
  ProviderError,
  ProviderEvent,
...
export class GenericProvider extends EventEmitter implements ProviderBase {
  ...
  public constructor(options: GenericProviderOptions) {
    super();
    this.encoder =
      typeof options.encoder !== "undefined" ? options.encoder : new Encoder();
    this.accountId = options.accountId;
    this.gatewayConfig = options.gatewayConfig;
    this.unsafeRecipientIds = options.unsafeRecipientIds;
    this.assetLedgerSource =
      options.assetLedgerSource ||
      "https://azizazlan.github.io/cryptocert-ethereum-conventions/xcert-mock.json";
    this.valueLedgerSource =
      options.valueLedgerSource ||
      "https://conventions.0xcert.org/ethereum/token-mock.json";
    this.signMethod =
      typeof options.signMethod !== "undefined"
        ? options.signMethod
        : SignMethod.ETH_SIGN;
    this.requiredConfirmations =
      typeof options.requiredConfirmations !== "undefined"
        ? options.requiredConfirmations
        : 1;
    this.mutationTimeout =
      typeof options.mutationTimeout !== "undefined"
        ? options.mutationTimeout
        : 3600000; // 1 h
    this.gasPriceMultiplier =
      typeof options.gasPriceMultiplier !== "undefined"
        ? options.gasPriceMultiplier
        : 1.1;
    this.retryGasPriceMultiplier =
      typeof options.retryGasPriceMultiplier !== "undefined"
        ? options.retryGasPriceMultiplier
        : 2;
    this.sandbox =
      typeof options.sandbox !== "undefined" ? options.sandbox : false;
    this.verbose =
      typeof options.verbose !== "undefined" ? options.verbose : false;

    this._client =
      options.client && options.client.currentProvider
  ...
  public set unsafeRecipientIds(ids: string[]) {
    this._unsafeRecipientIds = (ids || []).map((id) =>
      this.encoder.normalizeAddress(id)
    );
  }
  ...
  public set gatewayConfig(config: GatewayConfig) {
    if (typeof config !== "undefined") {
      this._gatewayConfig = {
        ...
        assetLedgerDeployOrderId: this.encoder.normalizeAddress(
          config.assetLedgerDeployOrderId
        ),
        valueLedgerDeployOrderId: this.encoder.normalizeAddress(
   ...
   */
  public emit(
    event: ProviderEvent.ACCOUNT_CHANGE,
    newAccountId: string,
    oldAccountId: string
  );
  public emit(
    event: ProviderEvent.NETWORK_CHANGE,
    newNetworkVersion: string,
    oldNetworkVersion: string
  );
  public emit(...args) {
  ...
  public on(
    event: ProviderEvent.ACCOUNT_CHANGE,
    handler: (newAccountId: string, oldAccountId: string) => any
  );
  public on(
    event: ProviderEvent.NETWORK_CHANGE,
    handler: (newNetworkVersion: string, oldNetworkVersion: string) => any
  );
  public on(...args) {
    ...
    if (payload.method === "eth_sendTransaction" && payload.params.length) {
      if (this.sandbox || typeof payload.params[0].gas === "undefined") {
        try {
          ...
          payload.params[0].gas = `0x${Math.ceil(res.result * 1.1).toString(
            16
          )}`;
        } catch (err) {
          if (
            err &&
            err.original &&
            (err.original.code === -32603 || err.original.code === -32000)
          ) {
        ...
        if (typeof payload.params[0].gasPrice === "undefined") {
          ...
          payload.params[0].gasPrice = `0x${Math.ceil(
            res.result * this.gasPriceMultiplier
          ).toString(16)}`;
        }
      }
    }
    ...
    return new Promise<RpcResponse>((resolve, reject) => {
      ...
      .then((value) => {
        ...
        if (
          ...
          value &&
          value.result &&
          value.result.indexOf("0x08c379a0") !== -1
        ) {
          const errorCode = this.encoder.decodeParameters(
            ["string"],
            `0x${value.result.substring(10)}`
          );
