import { normalizeAddress } from "@cryptocert/ethereum-utils";
import {
  MutationBase,
  MutationContext,
  MutationEvent,
...
export class Mutation extends EventEmitter implements MutationBase {
  ...
  public once(
    event: MutationEvent.ERROR,
    handler: (e: any, m: Mutation) => any
  );
  public once(...args) {
  ...
  public async retry() {
    ...
    const retryGasPrice =
      gasPrice.result * this._provider.retryGasPriceMultiplier;
    ...
    const newGasPrice =
      retryGasPrice >= oldGasPrice
        ? retryGasPrice
        : oldGasPrice * this._provider.retryGasPriceMultiplier;
    const attrs = {
      from: tx.from,
      data: tx.input,
      nonce: tx.nonce,
  ...
  public async cancel() {
    ...
    if (!tx) {
      ...
      tx.from.toLowerCase() !== this._provider.accountId.toLowerCase()
    ) {
      throw new Error(
        "You are not the maker of this mutation so you cannot cancel it."
      );
    }
  ...
  protected async resolveCurrentState() {
    ...
    if (tx && (!tx.to || tx.to === "0x0")) {
      tx.to = await this.getTransactionReceipt().then((r) =>
  ...
  protected async loopUntilCompleted() {
    ...
    if (
      this._provider.mutationTimeout === -1 ||
      Date.now() - this._started < this._provider.mutationTimeout
    ) {
  ...
  protected async parseLogs() {
    try {
      this._logs = [];
      const eventSignatures: MutationEventSignature[] =
      ...
      transactionReceipt.logs.forEach((log) => {
        const eventSignature = eventSignatures.find(
          (e) => e.topic === log.topics[0]
        );
        if (!eventSignature) {
        ...
        const normal = eventSignature.types.filter(
          (t) => t.kind === MutationEventTypeKind.NORMAL
        );
        const indexed = eventSignature.types.filter(
          (t) => t.kind === MutationEventTypeKind.INDEXED
        );
        if (normal.length > 0) {
          const normalTypes = normal.map((n) => n.type);
          const decoded = this._provider.encoder.decodeParameters(
            normalTypes,
            log.data
          );
        ...
        indexed.forEach((i, idx) => {
          obj[i.name] = this._provider.encoder.decodeParameters(
            [i.type],
            log.topics[idx + 1]
          )[0];
        });
