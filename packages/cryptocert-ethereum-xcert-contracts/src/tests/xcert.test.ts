...
spec.beforeEach(async (ctx) => {
  ...
  ctx.set(
    "digest2",
    "0x6f25b3f4bc7eadafb8f57d69f8a59db3b23f198151dbf3c66ac3082381518329"
  );
  ctx.set(
    "digest3",
    "0xc77a290be17f8a4ef301c4ca46497c5beb4a0556ec2d5a04dce4ce6ebd439ad1"
  );
});
...
spec.test("returns correct convention per token", async (ctx) => {
  ...
  const convention = await xcert.instance.methods
    .tokenURISchemaIntegrity(id)
    .call();
  ctx.is(
    convention.digest,
    "0x9c22ff5f21f0b81b113e63f7db6da94fedef11b2119b4088b89664fb9a3cb658"
...
spec.test("correctly checks all the supported interfaces", async (ctx) => {
  const xcert = ctx.get("xcert");
  const nftokenInterface = await xcert.instance.methods
    .supportsInterface("0x80ac58cd")
    .call();
  const nftokenMetadataInterface = await xcert.instance.methods
    .supportsInterface("0x5b5e139f")
    .call();
  const nftokenMetadataEnumerableInterface = await xcert.instance.methods
    .supportsInterface("0x780e9d63")
    .call();
  const nonExistingInterface = await xcert.instance.methods
    .supportsInterface("0xa40e9c95")
    .call();
...
spec.test(
  "throws when trying to create two xcerts with the same id",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
    ...
    await ctx.reverts(
      () =>
        xcert.instance.methods.create(bob, id, digest).send({ from: owner }),
      "006006"
    );
  }
);
...
spec.test("throws when a third party tries to create an Xcert", async (ctx) => {
  ...
  await ctx.reverts(
    () => xcert.instance.methods.create(bob, id, digest).send({ from: sara }),
    "017001"
  );
});

spec.test(
  "throws when trying to create an Xcert to zero address",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const owner = ctx.get("owner");
    ...
    await ctx.reverts(
      () =>
        xcert.instance.methods
          .create(zeroAddress, id, digest)
          .send({ from: owner }),
      "006001"
    );
  }
...
spec.test("corectly grants create ability", async (ctx) => {
  ...
  const logs = await xcert.instance.methods
    .grantAbilities(bob, XcertAbilities.CREATE_ASSET)
    .send({ from: owner });
  ctx.not(logs.events.SetAbilities, undefined);

  const bobHasAbility1 = await xcert.instance.methods
    .isAble(bob, XcertAbilities.CREATE_ASSET)
    .call();
  ctx.is(bobHasAbility1, true);
});

spec.test(
  "throws when a third party tries to grant create ability",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const bob = ctx.get("bob");
    const sara = ctx.get("sara");
...
spec.test(
  "correctly creates an Xcert from an address with create ability",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
...
spec.test(
  "throws trying to create from address which authorization got revoked",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
    ...
    await xcert.instance.methods
      ...
      .send({ from: owner });
    await xcert.instance.methods
      .revokeAbilities(bob, XcertAbilities.CREATE_ASSET)
      .send({ from: owner });
    await ctx.reverts(
      () => xcert.instance.methods.create(sara, id, digest).send({ from: bob }),
      "017001"
    );
  }
);
...
spec.test(
  "throws when trying to find owner of a non-existing Xcert",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const id = ctx.get("id1");

    await ctx.reverts(
      () => xcert.instance.methods.ownerOf(id).call(),
      "006002"
    );
  }
);
...
spec.test(
  "finds the correct amount of xcerts owned by account",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
...
spec.test(
  "throws when trying to get count of xcerts owned by zero address",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const zeroAddress = ctx.get("zeroAddress");
    await ctx.reverts(
      () => xcert.instance.methods.balanceOf(zeroAddress).call(),
      "006001"
    );
  }
);
...
spec.test(
  "throws when trying to find owner of non-existant Xcert",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const id = ctx.get("id1");
    await ctx.reverts(
...
spec.test(
  "throws when trying to get approval of non-existing Xcert",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const id1 = ctx.get("id1");

    await ctx.reverts(
      () => xcert.instance.methods.getApproved(id1).call(),
      "006002"
    );
  }
);
...
spec.test(
  "throws when trying to approve a Xcert from a third party",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
    ...
    await xcert.instance.methods
      .create(bob, id1, digest1)
      .send({ from: owner });
    await ctx.reverts(
      () => xcert.instance.methods.approve(sara, id1).send({ from: sara }),
      "006003"
    );
  }
);
...
spec.test("correctly sets an operator", async (ctx) => {
  ...
  await xcert.instance.methods.create(bob, id1, digest1).send({ from: owner });
  const logs = await xcert.instance.methods
    .setApprovalForAll(sara, true)
    .send({ from: bob });
  ctx.not(logs.events.ApprovalForAll, undefined);
  const isApprovedForAll = await xcert.instance.methods
    .isApprovedForAll(bob, sara)
    .call();
...
spec.skip("correctly sets an operator with signature", async (ctx) => {
  ...
  await zxc.instance.methods
    .approve(xcert.receipt._address, tokenAmount.toString())
    .send({ from: bob });
  const claim = await xcert.instance.methods
    .generateClaim(
      bob,
      sara,
      true,
      zxc.receipt._address,
  ...
  const logs = await xcert.instance.methods
    .setApprovalForAllWithSignature(
      bob,
      sara,
      true,
      zxc.receipt._address,
 ...
 */
spec.skip(
  "correctly sets an operator with signature when fee recipient is not specified",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const zxc = ctx.get("zxc");
    const owner = ctx.get("owner");
...
spec.test(
  "fails setting an operator with signature if signature kind is invalid",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const zxc = ctx.get("zxc");
    const owner = ctx.get("owner");
...
spec.test(
  "fails setting an operator with signature if signature is from a third party",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const zxc = ctx.get("zxc");
    const owner = ctx.get("owner");
 ...
 */
spec.skip(
  "fails setting an operator with signature if claim was already used",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const zxc = ctx.get("zxc");
    const owner = ctx.get("owner");
...
spec.test(
  "fails setting an operator with signature if claim has expired",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const zxc = ctx.get("zxc");
    const owner = ctx.get("owner");
...
spec.test(
  "sucessfully cancels set an operator with signature claim",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const zxc = ctx.get("zxc");
    const owner = ctx.get("owner");
    ...
    await zxc.instance.methods
      ...
      .send({ from: bob });
    await xcert.instance.methods
      .cancelSetApprovalForAllWithSignature(
        bob,
        sara,
        true,
        zxc.receipt._address,
...
spec.test(
  "fails canceling set an operator with signature claim when sender is not the owner",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const zxc = ctx.get("zxc");
    const owner = ctx.get("owner");
...
spec.test(
  "throws when trying to transfer Xcert as an address that is not owner, approved or operator",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
...
spec.test(
  "throws when trying to transfer Xcert to a zero address",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
    ...
    await ctx.reverts(
      () =>
        xcert.instance.methods
          .transferFrom(bob, zeroAddress, id1)
          .send({ from: bob }),
      "006001"
    );
  }
...
spec.test("throws when trying to transfer a invalid Xcert", async (ctx) => {
  ...
  await ctx.reverts(
    () =>
      xcert.instance.methods.transferFrom(bob, sara, id2).send({ from: bob }),
    "006002"
  );
});
...
spec.test(
  "throws when trying to safe transfers Xcert from owner to a smart contract",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
    ...
    await ctx.reverts(() =>
      xcert.instance.methods
        .safeTransferFrom(bob, xcert.receipt._address, id1)
        .send({ from: bob })
    );
  }
);
...
spec.test(
  "corectly safe transfers xcert from owner to smart contract that can recieve NTFs",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
    ...
    const tokenReceiver = await ctx.deploy({
      src: "cryptocert/ethereum-erc721-contracts/build/nf-token-receiver-test-mock.json",
    ...
    await xcert.instance.methods
      ...
      .send({ from: owner });
    await xcert.instance.methods
      .safeTransferFrom(bob, tokenReceiver.receipt._address, id1)
      .send({ from: bob });

    const bobBalance = await xcert.instance.methods.balanceOf(bob).call();
    const saraBalance = await xcert.instance.methods
      .balanceOf(tokenReceiver.receipt._address)
      .call();
    const ownerOfId1 = await xcert.instance.methods.ownerOf(id1).call();
...
spec.test(
  "corectly safe transfers xcert from owner to smart contract that can recieve NFTs with data",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
    ...
    await xcert.instance.methods
      ...
      .send({ from: owner });
    await xcert.instance.methods
      .safeTransferFrom(bob, tokenReceiver.receipt._address, id1, "0x01")
      .send({ from: bob });
...
spec.test("return the correct URI", async (ctx) => {
  ...
  const bigId = new ctx.web3.utils.BN(
    "115792089237316195423570985008687907853269984665640564039457584007913129639935"
  ).toString();
  await xcert.instance.methods
    .create(bob, bigId, digest3)
    .send({ from: owner });
...
spec.test("succesfully changes URI prefix", async (ctx) => {
  ...
  await xcert.instance.methods
    .setUri(newUriPrefix, uriPostfix)
    .send({ from: owner });
...
spec.test("throws when trying to get URI of invalid NFT ID", async (ctx) => {
  ...
  await ctx.reverts(
    () => xcert.instance.methods.tokenURI(id1).call(),
    "006002"
  );
});
...
spec.test(
  "throws when trying to get xcert by non-existing index",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
    ...
    await xcert.instance.methods
      ...
      .send({ from: owner });
    await ctx.reverts(
      () => xcert.instance.methods.tokenByIndex(1).call(),
      "006007"
    );
  }
);
...
spec.test("returns the correct token of owner by index", async (ctx) => {
  ...
  const tokenOwnerIndex1 = await xcert.instance.methods
    .tokenOfOwnerByIndex(bob, 1)
    .call();
  ctx.is(tokenOwnerIndex1, id2);
});

spec.test(
  "throws when trying to get xcert of owner by non-existing index",
  async (ctx) => {
    const xcert = ctx.get("xcert");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
    ...
    await xcert.instance.methods
      ...
      .send({ from: owner });
    await ctx.reverts(
      () => xcert.instance.methods.tokenOfOwnerByIndex(bob, 1).call(),
      "006007"
    );
  }
);
...
spec.test("returns the correct token uri integrity digest", async (ctx) => {
  ...
  await xcert.instance.methods.create(bob, id, digest).send({ from: owner });
  const xcertId1Imprint = await xcert.instance.methods
    .tokenURIIntegrity(id)
    .call();
...
spec.test("throws when trying to use revoke capability", async (ctx) => {
  ...
  await xcert.instance.methods.create(bob, id1, digest1).send({ from: owner });
  await ctx.reverts(
    () => xcert.instance.methods.revoke(id1).send({ from: owner }),
    "007001"
  );
});
...
spec.test("throws when trying to use pause capability", async (ctx) => {
  ...
  await ctx.reverts(
    () => xcert.instance.methods.setPause(true).send({ from: owner }),
    "007001"
  );
});
...
spec.test("throws when trying to use mutable capability", async (ctx) => {
  ...
  await ctx.reverts(
    () =>
      xcert.instance.methods
        .updateTokenURIIntegrityDigest(id1, newImprint)
        .send({ from: owner }),
    "007001"
  );
});
...
spec.test("throws when trying to use destroy capability", async (ctx) => {
  ...
  await xcert.instance.methods.create(bob, id1, digest1).send({ from: owner });
  await ctx.reverts(
    () => xcert.instance.methods.destroy(id1).send({ from: bob }),
    "007001"
  );
});
