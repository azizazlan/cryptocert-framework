...
export default async function (
  ...
  const functionSignature =
    typeof receiverData !== "undefined" ? "0xb88d4fde" : "0x42842e0e";
  const inputTypes = ["address", "address", "uint256"];
  if (typeof receiverData !== "undefined") {
    inputTypes.push("bytes");
  }
  const data = [senderId, receiverId, id, receiverData].filter(
  ...
  const attrs = {
    ...
    data:
      functionSignature +
      ledger.provider.encoder.encodeParameters(inputTypes, data).substr(2),
  };
