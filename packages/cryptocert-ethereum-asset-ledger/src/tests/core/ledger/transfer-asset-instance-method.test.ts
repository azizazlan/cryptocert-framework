...
spec.test("transfer asset to a contract", async (ctx) => {
  ...
  const ledger = ctx.get("ledger");
  const coinbase = ctx.get("coinbase");
  const nftokenReceiver =
    ctx.get("protocol").nftokenReceiver.instance.options.address;
  await xcert.instance.methods
    .create(
      coinbase,
      "2",
...
spec.test(
  "fails when trying to transfer asset to a contract that does not implement receiver",
  async (ctx) => {
    const xcert = ctx.get("protocol").xcert;
    const ledger = ctx.get("ledger");
    const coinbase = ctx.get("coinbase");
...
spec.test(
  "transfer to asset contract that does not implement receiver but is marked as unsafe",
  async (ctx) => {
    const xcert = ctx.get("protocol").xcert;
    const ledger = ctx.get("ledger");
    const coinbase = ctx.get("coinbase");
    const tokenTransferProxy =
      ctx.get("protocol").tokenTransferProxy.instance.options.address;
    await xcert.instance.methods
      .create(
        coinbase,
        "5",
    ...
    ctx.is(
      await xcert.instance.methods.ownerOf("5").call(),
      tokenTransferProxy
    );
  }
);
...
spec.test(
  "transfer to approved asset contract that does not implement receiver but is marked as unsafe",
  async (ctx) => {
    const xcert = ctx.get("protocol").xcert;
    const ledger = ctx.get("ledger");
    const coinbase = ctx.get("coinbase");
    ...
    ctx.is(
      await xcert.instance.methods.ownerOf("7").call(),
      tokenTransferProxy
    );
  }
);
