...
import { GeneralAssetLedgerAbility } from "@cryptocert/scaffold";
...
spec.test("sets ledger abilities for an account", async (ctx) => {
  const ledger = ctx.get("ledger");
  const bob = ctx.get("bob");
  const prepare = await ledger.grantAbilities(bob, [
    GeneralAssetLedgerAbility.REVOKE_ASSET,
    GeneralAssetLedgerAbility.TOGGLE_TRANSFERS,
  ]);
  await prepare.complete();
  const mutation = await ledger.setAbilities(bob, [
