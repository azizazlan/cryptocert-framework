...
spec.test(
  "returns null calling a contract that does not have getApprovedAccount function",
  async (ctx) => {
    const provider = ctx.get("provider");
...
spec.test("check crypto kitty fallback hack", async (ctx) => {
  ...
  const kittyHackBytecode =
    "";
  const kittyHashContract = new ctx.web3.eth.Contract(kittyHackAbi);
  let ledgerId;
  await kittyHashContract
    .deploy({ data: kittyHackBytecode })
  ...
  await kittyHashContract.methods
    .addToKittyIndex("1", coinbase)
    .send({ from: coinbase });
