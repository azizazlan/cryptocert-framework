...
spec.test("revokes ledger abilities for an account", async (ctx) => {
  ...
  await ledger
    ...
    .then(() => ctx.sleep(200));
  const mutation = await ledger.revokeAbilities(bob, [
    GeneralAssetLedgerAbility.UPDATE_URI_BASE,
  ]);
...
spec.test("revokes ledger super abilities for an account", async (ctx) => {
  ...
  await ledger
    .grantAbilities(jane, [SuperAssetLedgerAbility.MANAGE_ABILITIES])
    .then(() => ctx.sleep(200));
  await ledger
    .revokeAbilities(jane, [SuperAssetLedgerAbility.MANAGE_ABILITIES])
    .then(() => ctx.sleep(200));
...
spec.test("revokes own super ability", async (ctx) => {
  ...
  await ledger
    .revokeAbilities(owner, [SuperAssetLedgerAbility.MANAGE_ABILITIES])
    .then(() => ctx.sleep(200));
