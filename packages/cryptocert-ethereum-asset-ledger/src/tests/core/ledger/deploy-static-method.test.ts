...
spec.test("deploys new asset ledger", async (ctx) => {
  ...
  const capabilities = [AssetLedgerCapability.TOGGLE_TRANSFERS];
  const info = {
    name: "Foo",
    symbol: "Bar",
  ...
  const ledger = await AssetLedger.deploy(provider, { ...info, capabilities })
    .then((mutation) => {
