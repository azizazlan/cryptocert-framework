...
import {
  ...
  AssetLedgerDeployRecipe,
  AssetLedgerInfo,
  AssetLedgerItem,
  AssetLedgerItemRecipe,
  AssetLedgerObjectUpdateRecipe,
...
export class AssetLedger implements AssetLedgerBase {
  ...
  public static async deploy(
    ...
    recipe: AssetLedgerDeployRecipe
  ): Promise<Mutation> {
   ...
   * @param index Asset index.
   */
  public async getAccountAssetIdAt(
    accountId: string,
    index: number
  ): Promise<number> {
    accountId = this._provider.encoder.normalizeAddress(accountId);
   ...
   */
  public async isApprovedAccount(
    assetId: string,
    accountId: string
  ): Promise<boolean> {
  ...
  public async createAsset(recipe: AssetLedgerItemRecipe): Promise<Mutation> {
    const receiverId = this._provider.encoder.normalizeAddress(
      recipe.receiverId
    );
   ...
   * @param abilities List of the abilities.
   */
  public async revokeAbilities(
    accountId: string,
    abilities: AssetLedgerAbility[]
  ): Promise<Mutation> {
    accountId = this._provider.encoder.normalizeAddress(accountId as string);
    return revokeAbilities(
      this,
      accountId,
      getBitfieldFromAbilities(abilities)
    );
  }
  ...
  public async transferAsset(
    recipe: AssetLedgerTransferRecipe
  ): Promise<Mutation> {
    if (!recipe.senderId) {
  ...
  public async updateAsset(
    assetId: string,
    recipe: AssetLedgerObjectUpdateRecipe
  ): Promise<Mutation> {
   ...
   */
  public async isApprovedOperator(
    accountId: string,
    operatorId: string
  ): Promise<boolean> {
    accountId = this._provider.encoder.normalizeAddress(accountId);
  ...
  public getContext(): MutationEventSignature[] {
    return [
      ...
      },
      {
        ...
        topic:
          "0xda81dc3bdaf65b621535dbd08fc4032c7e22d5ccab12c8aa2ad010611471d627",
        types: [
          {
            kind: MutationEventTypeKind.INDEXED,
            name: "tokenId",
