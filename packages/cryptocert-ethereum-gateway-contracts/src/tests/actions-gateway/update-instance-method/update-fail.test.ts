import { XcertUpdateProxyAbilities } from "@cryptocert/ethereum-proxy-contracts/src/core/types";
...
spec.beforeEach(async (ctx) => {
  ...
  await cat.instance.methods.create(bob, id, imprint1).send({
    from: owner,
  });
  ctx.set("cat", cat);
});
...
spec.test(
  "throws when signer does not have ability to sign an update asset",
  async (ctx) => {
    const actionsGateway = ctx.get("actionsGateway");
...
spec.test(
  "throws when proxy does not have ability to update assets",
  async (ctx) => {
    const actionsGateway = ctx.get("actionsGateway");
    const jane = ctx.get("jane");
    const owner = ctx.get("owner");
