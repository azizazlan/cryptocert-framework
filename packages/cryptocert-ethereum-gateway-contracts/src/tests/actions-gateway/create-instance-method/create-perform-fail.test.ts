import { XcertCreateProxyAbilities } from "@cryptocert/ethereum-proxy-contracts/src/core/types";
...
spec.test(
  "fails if maker does not have asset creating ability",
  async (ctx) => {
    const actionsGateway = ctx.get("actionsGateway");
    const createProxy = ctx.get("createProxy");
    const jane = ctx.get("jane");
    ...
    await cat.instance.methods
      .revokeAbilities(owner, XcertAbilities.ALLOW_CREATE_ASSET)
      .send({ from: owner });
    await cat.instance.methods
      .grantAbilities(createProxy.receipt._address, XcertAbilities.CREATE_ASSET)
      .send({ from: owner });
