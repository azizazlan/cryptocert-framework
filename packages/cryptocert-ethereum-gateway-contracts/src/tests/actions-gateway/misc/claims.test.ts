...
spec.beforeEach(async (ctx) => {
  const cat = await ctx.deploy({
    src: "cryptocert/ethereum-erc721-contracts/build/nf-token-metadata-enumerable-mock.json",
...
spec.beforeEach(async (ctx) => {
  ...
  const hash = await actionsGateway.instance.methods
    .getOrderDataClaim(tuple)
    .call();
  ctx.set("hash", hash);
});
...
spec.test("check valid signature", async (ctx) => {
  ...
  const valid = await actionsGateway.instance.methods
    .isValidSignature(account, hash, tuple)
    .call();
  ctx.true(valid);
});
