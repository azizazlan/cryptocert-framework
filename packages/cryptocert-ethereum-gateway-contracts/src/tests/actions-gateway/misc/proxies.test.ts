...
spec.beforeEach(async (ctx) => {
  const nftProxy = await ctx.deploy({
    src: "cryptocert/ethereum-proxy-contracts/build/nftoken-transfer-proxy.json",
...
spec.test("correctly adds proxy addresses", async (ctx) => {
  ...
  const logs = await actionsGateway.instance.methods
    .addProxy(tokenProxy.receipt._address, 0)
    .send({ from: owner });
  ...
  await actionsGateway.instance.methods
    .addProxy(nftProxy.receipt._address, 0)
    .send({ from: owner });
...
spec.test("correctly removes proxy address", async (ctx) => {
  ...
  const addLogs = await actionsGateway.instance.methods
  ...
  const removeLogs = await actionsGateway.instance.methods
    .removeProxy(0)
    .send({ from: owner });
...
spec.test(
  "throws when a third party tries to add proxy address",
  async (ctx) => {
    const zeroAddress = ctx.get("zeroAddress");
