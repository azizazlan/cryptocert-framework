import { XcertBurnProxyAbilities } from "@cryptocert/ethereum-proxy-contracts/src/core/types";
...
spec.beforeEach(async (ctx) => {
  ...
  await cat.instance.methods.create(jane, id, digest1).send({
    from: owner,
  });
  ctx.set("cat", cat);
});
...
spec.beforeEach(async (ctx) => {
  ...
  await actionsGateway.instance.methods
    ...
    .send();
  await actionsGateway.instance.methods
    .addProxy(burnProxy.receipt._address, 4)
    .send({ from: owner });
...
spec.beforeEach(async (ctx) => {
  ...
  const owner = ctx.get("owner");
  await burnProxy.instance.methods
    .grantAbilities(
      actionsGateway.receipt._address,
      XcertBurnProxyAbilities.EXECUTE
    )
    .send({ from: owner });
});
...
spec.test("throws when signer is not the owner", async (ctx) => {
  ...
  await cat.instance.methods
    .setApprovalForAll(burnProxy.receipt._address, true)
    .send({ from: jane });
  await ctx.reverts(
    () =>
...
spec.test(
  "throws when proxy does not have approval do destory an asset",
  async (ctx) => {
    const actionsGateway = ctx.get("actionsGateway");
    const jane = ctx.get("jane");
    const owner = ctx.get("owner");
