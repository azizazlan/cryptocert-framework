import { TokenTransferProxyAbilities } from "@cryptocert/ethereum-proxy-contracts/src/core/types";
...
spec.beforeEach(async (ctx) => {
  const deployProxy = await ctx.deploy({
    src: "cryptocert/ethereum-proxy-contracts/build/xcert-deploy-proxy.json",
...
spec.beforeEach(async (ctx) => {
  const burnProxy = await ctx.deploy({
    src: "cryptocert/ethereum-proxy-contracts/build/xcert-burn-proxy.json",
...
spec.beforeEach(async (ctx) => {
  ...
  await tokenProxy.instance.methods
    .grantAbilities(
      xcertDeployGateway.receipt._address,
...
spec.test("performs a deploy", async (ctx) => {
  ...
  const claim = await xcertDeployGateway.instance.methods
    .getDeployDataClaim(createTuple)
    .call();
  ...
  await zxc.instance.methods
    .approve(tokenProxy.receipt._address, 10000)
    .send({ from: jane });
  const logs = await xcertDeployGateway.instance.methods
    .performAnyTaker(createTuple, signatureDataTuple)
    .send({ from: owner });
  ...
  await xcert.methods
    .create(jane, "1", "0x1234567890123456789012345678901234567891")
    .send({
      from: jane,
      gas: "1000000",
    });
  ...
  const isAble = await xcert.methods
    .isAble(createProxy.receipt._address, XcertAbilities.CREATE_ASSET)
    .call();
  ctx.true(isAble);

  const supportsInterface = await xcert.methods
    .supportsInterface("0xbedb86fb")
    .call();
  ...
  const nftSafeTransferProxyApproved = await xcert.methods
    .isApprovedForAll(jane, nftTransferProxy.receipt._address)
    .call();
  ctx.true(nftSafeTransferProxyApproved);

  const xcertBurnProxyApproved = await xcert.methods
    .isApprovedForAll(jane, burnProxy.receipt._address)
    .call();
...
spec.test("fails when not enough balance", async (ctx) => {
  ...
  await zxc.instance.methods
    .approve(tokenProxy.receipt._address, 9000)
    .send({ from: jane });
  await ctx.reverts(
    () =>
      xcertDeployGateway.instance.methods
        .performAnyTaker(createTuple, signatureDataTuple)
        .send({ from: owner }),
    "001002"
  );
...
spec.test("fails with invalid signature", async (ctx) => {
  ...
  const claim = await xcertDeployGateway.instance.methods
    .getDeployDataClaim(invalidTuple)
    .call();
...
spec.test(
  "fails trying to perform an already performed deploy",
  async (ctx) => {
    const xcertDeployGateway = ctx.get("xcertDeployGateway");
    const zxc = ctx.get("zxc");
    const tokenProxy = ctx.get("tokenProxy");
    ...
    await zxc.instance.methods
      .approve(tokenProxy.receipt._address, 10000)
      .send({ from: jane });
    await xcertDeployGateway.instance.methods
      .performAnyTaker(createTuple, signatureDataTuple)
      .send({ from: owner });
    await ctx.reverts(
      () =>
