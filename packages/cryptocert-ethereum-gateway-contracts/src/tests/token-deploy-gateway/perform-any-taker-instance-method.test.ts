...
spec.beforeEach(async (ctx) => {
  const tokenDeployProxy = await ctx.deploy({
    src: "cryptocert/ethereum-proxy-contracts/build/token-deploy-proxy.json",
...
spec.beforeEach(async (ctx) => {
  ...
  const tokenDeployGateway = await ctx.deploy({
    ...
    args: [tokenDeployProxy.receipt._address, tokenProxy.receipt._address],
  });
...
spec.beforeEach(async (ctx) => {
  ...
  await tokenProxy.instance.methods
    .grantAbilities(
      tokenDeployGateway.receipt._address,
...
spec.test("performs a deploy", async (ctx) => {
  ...
  const claim = await tokenDeployGateway.instance.methods
  ...
  await zxc.instance.methods
    ...
    .send({ from: jane });
  const logs = await tokenDeployGateway.instance.methods
...
spec.test("fails when not enough balance", async (ctx) => {
  ...
  await ctx.reverts(
    () =>
      tokenDeployGateway.instance.methods
        .performAnyTaker(createTuple, signatureDataTuple)
        .send({ from: owner }),
    "001002"
  );
...
spec.test(
  ...
  async (ctx) => {
    ...
    await zxc.instance.methods
      .approve(tokenProxy.receipt._address, 10000)
      .send({ from: jane });
    await tokenDeployGateway.instance.methods
      .performAnyTaker(createTuple, signatureDataTuple)
      .send({ from: owner });
    await ctx.reverts(
      () =>
