...
spec.test("adds authorized address", async (ctx) => {
  ...
  const logs = await nftProxy.instance.methods
    .grantAbilities(bob, NFTokenSafeTransferProxyAbilities.EXECUTE)
    .send({ from: owner });
  ctx.not(logs.events.SetAbilities, undefined);

  const bobHasAbilityToExecute = await nftProxy.instance.methods
    .isAble(bob, NFTokenSafeTransferProxyAbilities.EXECUTE)
    .call();
...
spec.test("removes authorized address", async (ctx) => {
  ...
  const owner = ctx.get("owner");
  const bob = ctx.get("bob");
  await nftProxy.instance.methods
    .grantAbilities(bob, NFTokenSafeTransferProxyAbilities.EXECUTE)
    .send({ from: owner });
  const logs = await nftProxy.instance.methods
    .revokeAbilities(bob, NFTokenSafeTransferProxyAbilities.EXECUTE)
    .send({ from: owner });
  ctx.not(logs.events.SetAbilities, undefined);
...
spec.test("transfers an NFT", async (ctx) => {
  ...
  await cat.instance.methods.create(jane, 1).send({
    from: owner,
    gas: 4000000,
  });

  await cat.instance.methods
    .approve(nftProxy.receipt._address, 1)
    .send({ from: jane });
  await nftProxy.instance.methods
    .execute(cat.receipt._address, jane, sara, 1)
    .send({ from: bob });
...
spec.test("transfers an NFT with high ID", async (ctx) => {
  ...
  const newOwner = await cat.instance.methods
    .ownerOf(
      bigNumberify(
        "0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFE"
      )
...
spec.test(
  "fails when transferring to a contract without receiver",
  async (ctx) => {
    const nftProxy = ctx.get("nftProxy");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
    ...
    await ctx.reverts(() =>
      nftProxy.instance.methods
        .execute(cat.receipt._address, jane, nftProxy.receipt._address, 1)
        .send({ from: bob })
    );
  }
);
