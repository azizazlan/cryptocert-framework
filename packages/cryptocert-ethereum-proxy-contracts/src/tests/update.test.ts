...
spec.test("adds authorized address", async (ctx) => {
  ...
  const logs = await xcertUpdateProxy.instance.methods
    .grantAbilities(bob, XcertCreateProxyAbilities.EXECUTE)
    .send({ from: owner });
  ctx.not(logs.events.SetAbilities, undefined);

  const bobHasAbilityToExecute = await xcertUpdateProxy.instance.methods
    .isAble(bob, XcertCreateProxyAbilities.EXECUTE)
    .call();
...
spec.test("removes authorized address", async (ctx) => {
  ...
  const owner = ctx.get("owner");
  const bob = ctx.get("bob");
  await xcertUpdateProxy.instance.methods
    .grantAbilities(bob, XcertCreateProxyAbilities.EXECUTE)
    .send({ from: owner });
  const logs = await xcertUpdateProxy.instance.methods
    .revokeAbilities(bob, XcertCreateProxyAbilities.EXECUTE)
    .send({ from: owner });
  ctx.not(logs.events.SetAbilities, undefined);
...
spec.test("updates an Xcert", async (ctx) => {
  ...
  const imprint =
    "0x16ba121a04d5ad3b885f09af5c6c08b68766ce43d990f8420dc97f6ff485cb37";
  const newURIIntegrityDigest =
    "0x6108466288093cf38a5b155b015dc556cb23dd48e3bf4739aef4932541db8eea";
  ...
  await cat.instance.methods
    .grantAbilities(
      xcertUpdateProxy.receipt._address,
      XcertAbilities.UPDATE_ASSET_IMPRINT
    )
    .send({ from: owner });
  await cat.instance.methods.create(jane, 1, imprint).send({ from: owner });
  let token1URIIntegrityDigest = await cat.instance.methods
    .tokenURIIntegrity(1)
    .call();
  ctx.is(token1URIIntegrityDigest.digest, imprint);
  await xcertUpdateProxy.instance.methods
    .update(cat.receipt._address, 1, newURIIntegrityDigest)
    .send({ from: bob });

  token1URIIntegrityDigest = await cat.instance.methods
    .tokenURIIntegrity(1)
    .call();
...
spec.test(
  "fails if update is triggered by an unauthorized address",
  async (ctx) => {
    const xcertUpdateProxy = ctx.get("xcertUpdateProxy");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
    ...
    await ctx.reverts(
      () =>
        xcertUpdateProxy.instance.methods
          .update(cat.receipt._address, 1, newURIIntegrityDigest)
          .send({ from: bob }),
      "017001"
    );
