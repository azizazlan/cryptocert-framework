...
spec.test("adds authorized address", async (ctx) => {
  ...
  const logs = await xcertProxy.instance.methods
    .grantAbilities(bob, XcertCreateProxyAbilities.EXECUTE)
    .send({ from: owner });
  ctx.not(logs.events.SetAbilities, undefined);

  const bobHasAbilityToExecute = await xcertProxy.instance.methods
...
spec.test("removes authorized address", async (ctx) => {
  ...
  const owner = ctx.get("owner");
  const bob = ctx.get("bob");
  await xcertProxy.instance.methods
    .grantAbilities(bob, XcertCreateProxyAbilities.EXECUTE)
    .send({ from: owner });
  const logs = await xcertProxy.instance.methods
...
spec.test("creates an Xcert", async (ctx) => {
  ...
  await cat.instance.methods
    .grantAbilities(xcertProxy.receipt._address, XcertAbilities.CREATE_ASSET)
    .send({ from: owner });
  await xcertProxy.instance.methods
    .create(cat.receipt._address, jane, 1, "0x0")
    .send({ from: bob });
...
spec.test(
  "fails if create is triggered by an unauthorized address",
  async (ctx) => {
    const xcertProxy = ctx.get("xcertProxy");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
