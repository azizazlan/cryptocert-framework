...
spec.test("adds authorized address", async (ctx) => {
  ...
  const logs = await xcertBurnProxy.instance.methods
    .grantAbilities(bob, XcertCreateProxyAbilities.EXECUTE)
    .send({ from: owner });
  ctx.not(logs.events.SetAbilities, undefined);

  const bobHasAbilityToExecute = await xcertBurnProxy.instance.methods
...
spec.test("removes authorized address", async (ctx) => {
  ...
  const owner = ctx.get("owner");
  const bob = ctx.get("bob");
  await xcertBurnProxy.instance.methods
    .grantAbilities(bob, XcertCreateProxyAbilities.EXECUTE)
    .send({ from: owner });
  const logs = await xcertBurnProxy.instance.methods
...
spec.test("destroys an Xcert", async (ctx) => {
  ...
  await cat.instance.methods
    .setApprovalForAll(xcertBurnProxy.receipt._address, true)
    .send({ from: jane });
  await xcertBurnProxy.instance.methods
    .destroy(cat.receipt._address, 1)
    .send({ from: bob });
  await ctx.reverts(() => cat.instance.methods.ownerOf(1).call());
});

spec.test(
  "fails if destroy is triggered by an unauthorized address",
  async (ctx) => {
    const xcertBurnProxy = ctx.get("xcertBurnProxy");
    const owner = ctx.get("owner");
    const bob = ctx.get("bob");
    ...
    await cat.instance.methods.create(jane, 1, imprint).send({ from: owner });
    await ctx.reverts(() =>
      xcertBurnProxy.instance.methods
        .destroy(cat.receipt._address, 1)
        .send({ from: bob })
    );
  }
