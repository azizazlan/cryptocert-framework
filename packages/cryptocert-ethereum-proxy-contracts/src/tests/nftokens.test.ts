...
spec.test("adds authorized address", async (ctx) => {
  ...
  const logs = await nftProxy.instance.methods
    .grantAbilities(bob, NFTokenTransferProxyAbilities.EXECUTE)
    .send({ from: owner });
  ctx.not(logs.events.SetAbilities, undefined);

  const bobHasAbilityToExecute = await nftProxy.instance.methods
    .isAble(bob, NFTokenTransferProxyAbilities.EXECUTE)
    .call();
...
spec.test("removes authorized address", async (ctx) => {
  ...
  await nftProxy.instance.methods
    ...
    .send({ from: owner });
  const logs = await nftProxy.instance.methods
    .revokeAbilities(bob, NFTokenTransferProxyAbilities.EXECUTE)
    .send({ from: owner });
  ctx.not(logs.events.SetAbilities, undefined);
