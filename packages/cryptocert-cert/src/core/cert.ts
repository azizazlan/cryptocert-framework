import { Schema87 } from "@cryptocert/conventions";
import { Merkle, MerkleHasher, MerkleNoncer } from "@cryptocert/merkle";
  ...
  protected buildSchemaProps(
    ...
    if (schema.type === "array") {
      ...
      return Object.keys(schema.properties)
        .sort()
        .map((key: string) => {
          const prop = this.buildSchemaProps(
            (data || {})[key],
            schema.properties[key],
            [...prepend, key]
          );
          return ["object", "array"].indexOf(schema.properties[key].type) === -1
            ? [prop]
            : prop;
        })
        .reduce((a, b) => a.concat(b), []);
    ...
    for (const recipe of recipes) {
      ...
      if (groupRecipe) {
        groupRecipe.values.unshift({
          // adds posible duplicate thus use `unshift`
          index: groupIndex,
          value: imprint,
