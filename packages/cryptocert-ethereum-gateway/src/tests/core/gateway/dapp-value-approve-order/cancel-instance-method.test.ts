...
import { DappValueApproveOrder, OrderKind } from "cryptocert/scaffold";
...
spec.before(async (stage) => {
  ...
  await dappToken.instance.methods
    .grantAbilities(coinbase, 16)
    .send({ form: coinbase });
  await dappToken.instance.methods
    .setWhitelistedRecipient(bob, true)
    .send({ form: coinbase });
  await dappToken.instance.methods
    .deposit(100000, coinbase)
    .send({ form: coinbase });
});

spec.test("sucessfully cancels DappValueApproveOrder", async (ctx) => {
  ...
  const dappTokenAddress =
    ctx.get("protocol").dappToken.instance.options.address;
  ...
  const mutation = await gatewayCoinbase.cancel(order);
  await mutation.complete();
  await ctx.throws(() => gatewayBob.perform(order, sign));
});
