...
spec.test(
  "marks tokenDeployGateway order as canceled on the network which prevents the deploy from being performed",
  async (ctx) => {
    ...
    const tokenDeployGatewayId =
      ctx.get("protocol").tokenDeployGateway.instance.options.address;

    const tokenDeployGatewayBob = new Gateway(bobGenericProvider, {
      actionsOrderId: "",
    ...
    const xcertDeployGatewayCoinbase = new Gateway(
      coinbaseGenericProvider,
      tokenDeployGatewayId
    );
