...
import { OrderKind, ValueLedgerDeployOrder } from "cryptocert/scaffold";
...
spec.before(async (stage) => {
  ...
  const tokenDeployGatewayId =
    stage.get("protocol").tokenDeployGateway.instance.options.address;
  ...
  const coinbaseGenericProvider = new GenericProvider({
    ...
    gatewayConfig: {
      ...
      valueLedgerDeployOrderId: tokenDeployGatewayId,
    },
...
spec.test(
  "submits gateway value ledger deploy order to the network which executes transfer and creates a new token",
  async (ctx) => {
    ...
    ctx.is(
      await token.instance.methods.balanceOf(bob).call(),
...
spec.test(
  "submits dynamic value ledger deploy order to the network which executes transfer and creates a new token",
  async (ctx) => {
    const bob = ctx.get("bob");
    const token = ctx.get("protocol").erc20;
    const tokenId = ctx.get("protocol").erc20.instance.options.address;
    ...
    const tokenAddress = gatewayCoinbase.provider.encoder.decodeParameters(
      ["address", "bytes32"],
      performEvent[0].data
    )[0];
...
spec.test(
  "handles fixed deploy with dynamic token transfer receiver",
  async (ctx) => {
    const coinbase = ctx.get("coinbase");
    const bob = ctx.get("bob");
    const tokenId = ctx.get("protocol").erc20.instance.options.address;
