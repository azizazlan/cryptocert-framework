...
spec.before(async (stage) => {
  ...
  await xcert.instance.methods
    ...
    .send({ form: coinbase });
  await xcert.instance.methods
    .approve(nftokenSafeTransferProxy, "100")
    .send({ from: sara });
  await xcert.instance.methods
    .approve(nftokenSafeTransferProxy, "101")
    .send({ from: jane });
});
...
spec.test(
  "marks gateway order as canceled on the network which prevents transfers from being swapped",
  async (ctx) => {
    const actionsGatewayId =
      ctx.get("protocol").actionsGateway.instance.options.address;
    ...
    const makerGateway = new Gateway(makerGenericProvider, {
      actionsOrderId: actionsGatewayId,
      assetLedgerDeployOrderId: "",
      valueLedgerDeployOrderId: "",
    });
    ...
    const takerGateway = new Gateway(takerGenericProvider, {
      actionsOrderId: actionsGatewayId,
      assetLedgerDeployOrderId: "",
      valueLedgerDeployOrderId: "",
    });
    await ctx.throws(() =>
      takerGateway.perform(order, [sign]).then(() => ctx.sleep(200))
    );
