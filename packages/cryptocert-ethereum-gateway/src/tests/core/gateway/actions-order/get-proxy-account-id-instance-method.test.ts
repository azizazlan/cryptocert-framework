...
spec.test("returns proxy account address", async (ctx) => {
  ...
  const tokenTransferProxy = await gateway.getProxyAccountId(
    ProxyKind.TRANSFER_TOKEN
  );
  const nftokenTransferProxy = await gateway.getProxyAccountId(
    ProxyKind.TRANSFER_ASSET,
    bob
  );
  const nftokenSafeTransferProxy = await gateway.getProxyAccountId(
    ProxyKind.TRANSFER_ASSET
  );
  const xcertCreateProxy = await gateway.getProxyAccountId(
    ProxyKind.CREATE_ASSET
  );

  ctx.is(
    tokenTransferProxy,
    protocol.tokenTransferProxy.instance.options.address
  );
  ctx.is(
    nftokenTransferProxy,
    protocol.nftokenTransferProxy.instance.options.address
  );
  ctx.is(
    nftokenSafeTransferProxy,
    protocol.nftokenSafeTransferProxy.instance.options.address
  );
  ctx.is(xcertCreateProxy, protocol.xcertCreateProxy.instance.options.address);
});

spec.test(
  "returns null when calling getProxyAccountId on a contract that does not support it",
  async (ctx) => {
    const protocol = ctx.get("protocol");
    const provider = ctx.get("provider");
