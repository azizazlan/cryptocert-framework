...
spec.before(async (stage) => {
  ...
  const xcertDeployGatewayId =
    stage.get("protocol").xcertDeployGateway.instance.options.address;
...
spec.test(
  "submits gateway asset ledger deploy order to the network which executes transfer and creates a new xcert",
  async (ctx) => {
    ...
    const xcertAddress = gatewayCoinbase.provider.encoder.decodeParameters(
      ["address", "bytes32"],
      performEvent[0].data
    )[0];
...
spec.test(
  "submits dynamic asset ledger deploy order to the network which executes transfer and creates a new xcert",
  async (ctx) => {
    const bob = ctx.get("bob");
    const token = ctx.get("protocol").erc20;
    const tokenId = ctx.get("protocol").erc20.instance.options.address;
