...
import { AssetSetOperatorOrder, OrderKind } from "cryptocert/scaffold";
...
spec.test("sucessfully performs AssetSetOperatorOrder", async (ctx) => {
  ...
  const xcertAddress = ctx.get("protocol").xcert.instance.options.address;
  const xcert = ctx.get("protocol").xcert;

  const order: AssetSetOperatorOrder = {
    kind: OrderKind.ASSET_SET_OPERATOR_ORDER,
    ledgerId: xcertAddress,
    owner: bob,
    operator: coinbase,
    isOperator: true,
    tokenTransferData: {
  ...
  await gatewayCoinbase.perform(order, sign);
  const isOperator = await xcert.instance.methods.isApprovedForAll(
    bob,
    coinbase
  );
  ctx.true(isOperator);
