import {
  ...
  GenericProvider,
  Mutation,
  MutationEventSignature,
  MutationEventTypeKind,
  SignMethod,
} from "cryptocert/ethereum-generic-provider";
import { ZERO_ADDRESS } from "cryptocert/ethereum-utils";
import {
  ...
  ValueLedgerDeployOrder,
} from "cryptocert/scaffold";
import {
  createOrderHash as createActionsOrderHash,
  normalizeOrderIds as normalizeActionsOrderIds,
} from "../lib/actions-order";
import {
  createOrderHash as createAssetLedgerDeployOrderHash,
  normalizeOrderIds as normalizeAssetLedgerDeployOrderIds,
} from "../lib/asset-ledger-deploy-order";
import {
  createOrderHash as createAssetSetOperatorOrderHash,
  normalizeOrderIds as normalizeAssetSetOperatorOrderIds,
} from "../lib/asset-set-operator-order";
import {
  createOrderHash as createDappValueApproveOrderHash,
  normalizeOrderIds as normalizeDappValueApproveOrderIds,
} from "../lib/dapp-value-approve-order";
import {
  createOrderHash as createValueLedgerDeployOrderHash,
  normalizeOrderIds as normalizeValueLedgerDeployOrderIds,
} from "../lib/value-ledger-deploy-order";
...
export class Gateway implements GatewayBase {
  ...
  public set config(config: GatewayConfig) {
    if (config) {
      this._config = {
        actionsOrderId: this._provider.encoder.normalizeAddress(
          config.actionsOrderId
        ),
        assetLedgerDeployOrderId: this._provider.encoder.normalizeAddress(
          config.assetLedgerDeployOrderId
        ),
        valueLedgerDeployOrderId: this._provider.encoder.normalizeAddress(
          config.valueLedgerDeployOrderId
        ),
      };
    } else {
      this._config = null;
  ...
  public async hash(order: Order): Promise<string> {
    if (
      order.kind === OrderKind.DYNAMIC_ACTIONS_ORDER ||
      ...
      order.kind === OrderKind.FIXED_ACTIONS_ORDER ||
  ...
  public async perform(
    order: DynamicActionsOrder,
    signature: string[]
  ): Promise<Mutation>;
  public async perform(
    order: SignedDynamicActionsOrder,
    signature: string[]
  ): Promise<Mutation>;
  public async perform(
  ...
  public async perform(
    order: ValueLedgerDeployOrder,
    signature: string
  ): Promise<Mutation>;
  public async perform(
    order: AssetSetOperatorOrder,
    signature: string
  ): Promise<Mutation>;
  public async perform(
    order: DappValueApproveOrder,
    signature: string
  ): Promise<Mutation>;
    ...
    if (order.kind === OrderKind.DYNAMIC_ACTIONS_ORDER) {
      ...
      if (order.signers.length !== signature.length) {
        throw new ProviderError(
          ProviderIssue.SIGNED_DYNAMIC_ACTIONS_ORDER_SIGNATURES
        );
      }
      order = normalizeActionsOrderIds(order, this._provider);
      return actionsOrderPerform(this, order, signature as string[]);
      ...
      if (order.signers.length !== signature.length) {
        throw new ProviderError(
          ProviderIssue.SIGNED_FIXED_ACTIONS_ORDER_SIGNATURES
        );
      }
      order = normalizeActionsOrderIds(order, this._provider);
      return actionsOrderPerform(this, order, signature as string[]);
   ...
   */
  public async getProxyAccountId(
    proxyKind: ProxyKind.TRANSFER_ASSET,
    ledgerId?: string
  );
    ...
    switch (args[0]) {
      case ProxyKind.TRANSFER_ASSET: {
        if (
          ...
          this.provider.unsafeRecipientIds.indexOf(args[1]) !== -1
        ) {
  ...
  public getContext(): MutationEventSignature[] {
    return [
      ...
      },
      {
        ...
        topic:
          "0xa4be90ab47bcea0c591eaa7dd28b8ba0329e7ebddac48c5f2ca9fed68d08cf08",
        types: [
          {
            kind: MutationEventTypeKind.INDEXED,
            name: "claim",
      ...
      },
      {
        ...
        topic:
          "0xe8d9861dbc9c663ed3accd261bbe2fe01e0d3d9e5f51fa38523b265c7757a93a",
        types: [
          {
            kind: MutationEventTypeKind.INDEXED,
            name: "claim",
   ...
   */
  protected createDynamicOrder(
    order: DynamicActionsOrder | SignedDynamicActionsOrder
  ) {
