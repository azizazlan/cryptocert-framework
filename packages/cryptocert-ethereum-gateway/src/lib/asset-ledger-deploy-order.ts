import { getInterfaceCode } from "cryptocert/ethereum-asset-ledger";
import {
  GenericProvider,
  SignMethod,
...
export function normalizeOrderIds(
  ...
  order.assetLedgerData.ownerId = provider.encoder.normalizeAddress(
    order.assetLedgerData.ownerId
  );
  return order;
}
