  ...
  const tokenData = keccak256(
    hexToBytes(
      [
        ...
        leftPad(
          bigNumberify(order.valueLedgerData.decimals).toHexString(),
          2,
          "0",
          false
        ),
...
export function normalizeOrderIds(
  ...
  order.tokenTransferData.ledgerId = provider.encoder.normalizeAddress(
  ...
  if (!order.tokenTransferData.receiverId) {
    ...
    order.tokenTransferData.receiverId = provider.encoder.normalizeAddress(
      order.tokenTransferData.receiverId
    );
  }
  order.valueLedgerData.ownerId = provider.encoder.normalizeAddress(
    order.valueLedgerData.ownerId
  );
  return order;
}
