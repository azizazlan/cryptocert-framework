import { getBitfieldFromAbilities } from "cryptocert/ethereum-asset-ledger";
import {
  GenericProvider,
  SignMethod,
...
import {
  ...
  rightPad,
  ZERO_ADDRESS,
} from "cryptocert/ethereum-utils";
import {
  ActionsOrder,
  ActionsOrderAction,
...
import { keccak256, toInteger, toSeconds, toTuple } from "cryptocert/utils";
...
export function getActionParams(action: ActionsOrderAction, signers: string[]) {
  ...
  if (action.kind == ActionsOrderActionKind.CREATE_ASSET) {
    ...
    params += leftPad(
      bigNumberify(action["assetId"]).toHexString(),
      64,
      "0",
      false
    );
    ...
    params = leftPad(
      bigNumberify(action["value"]).toHexString(),
      64,
      "0",
      true
    );
...
export function createSignatureTuple(claim: string[] | string) {
  if (typeof claim === "string") {
    ...
    const k =
      parseInt(kind) == SignMethod.PERSONAL_SIGN ? SignMethod.ETH_SIGN : kind;
    const signatureData = {
  ...
  if (
    ...
    order.actions.forEach((action) => {
      ...
      if (
        ...
        action.kind !== ActionsOrderActionKind.DESTROY_ASSET
      ) {
        action["receiverId"] = provider.encoder.normalizeAddress(
          action["receiverId"]
        );
      }
      action["senderId"] = provider.encoder.normalizeAddress(
        action["senderId"]
      );
    });
  } else {
    order.actions.forEach((action) => {
      action.ledgerId = provider.encoder.normalizeAddress(action.ledgerId);
      action["senderId"] = action["senderId"]
        ? provider.encoder.normalizeAddress(action["senderId"])
        : (action["senderId"] = ZERO_ADDRESS);
      if (
        action.kind !== ActionsOrderActionKind.UPDATE_ASSET_IMPRINT &&
        action.kind !== ActionsOrderActionKind.DESTROY_ASSET
      ) {
        action["receiverId"] = action["receiverId"]
          ? provider.encoder.normalizeAddress(action["receiverId"])
          : (action["receiverId"] = ZERO_ADDRESS);
        if (
          action["senderId"] === ZERO_ADDRESS &&
          action["receiverId"] === ZERO_ADDRESS
        ) {
          throw new ProviderError(
            ProviderIssue.SENDER_ID_AND_RECEIVER_ID_MISSING
          );
        }
      }
    });
